...
$(function(){
	...
	$("#btndel").click(function(){
		...
		if(len==0){
			...
			//삭제파일로 전송, nums로 보내기
			location.href="event/alldelete.jsp?nums="+n;
			location.reload();
		}
...
<%-- <h5 class="alert alert-info">
	...
	<div style="margin: 150px auto; width: 800px;height:auto;">
			...
			%>
				...
				<img src="save/<%=dto.getMainphoto()%>" width="800px" height="300px" border="1" onclick="location.href='index.jsp?main=event/contentview.jsp?num=<%=dto.getNum() %>&currentPage=<%=currentPage %>'" style="cursor:pointer"><br><br>
				<span style="margin-left:10px"><%=no--  %></span><span class="glyphicon glyphicon-eye-open" style="float:right;right:10px;"><%=dto.getReadcount()%></span>
				
				<%				
				if(dto.getAnswercount()>0){
					%>
					&nbsp;&nbsp;<a href="index.jsp?main=event/contentview.jsp?num=<%=dto.getNum()%>&currentPage=<%=currentPage %>#alist" style="color:#FF7BAC;text-decoration:none;"><span class="glyphicon glyphicon-menu-down"></span>&nbsp;<b><%=dto.getAnswercount() %></b></a>
					<%
				}
	...
	<div style="width: 800px; text-align: center;" class="container">
		<ul class="pagination">
			...
			if (startPage > 1) {
			%>
			<li><a href="index.jsp?main=event/smartlist.jsp?currentPage=<%=startPage - 1%>">이전</a>
			</li>
			<%
			}
			...
			if (pp == currentPage) {
			%>
			<li class="active"><a href="index.jsp?main=event/smartlist.jsp?currentPage=<%=pp%>" style="background-color:#FF7BAC;border:1px solid #FF7BAC;color:#ffffff;cursor:pointer;"><%=pp%></a>
			</li>
			<%
			} else {
			%>
			<li class="active"><a href="index.jsp?main=event/smartlist.jsp?currentPage=<%=pp%>" style="background-color:#FFffff;border:1px solid #FF7BAC;color:#FF7BAC;cursor:pointer;"><%=pp%></a>
			</li>
			<%
			}
			}
			...
			if (endPage < totalPage) {
			%>
			<li><a href="index.jsp?main=event/smartlist.jsp?currentPage=<%=endPage+1%>">다음</a></li>
			<%
             }
            %>
		</ul>
